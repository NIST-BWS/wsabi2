...
* This has all of the Objective-C categories (equiv to extension methods) for all of wsabi.
	* NSManagedObject+DeepCopy is (theoretically) a method to deep copy any Core Data object. It kind of works.
	...
	* The other UIImage categories are all from http://vocaro.com/trevor/blog/2009/10/12/resize-a-uiimage-the-right-way/
...
* The Symbolicons subgroup contains icons from the Symbolicons set, which we purchased from Symbolicons.com (Pat has login & license info)
* The "from iOS SDK" images are extracted from the iOS SDK using a tool called UIKit-extractor (available at https://github.com/0xced/UIKit-Artwork-Extractor)
